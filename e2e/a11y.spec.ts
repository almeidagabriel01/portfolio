...
/**
 * SEC-19: nenhuma das seções da v3 anima com `prefers-reduced-motion`.
 *
 * As seções da home nascem escondidas e sobem por transição (`useSectionReveal`);
 * com movimento reduzido elas têm que chegar prontas. Medir **opacidade 1 e
 * `transition-property: none`** cobre os dois defeitos possíveis: a seção que
 * fica invisível esperando um reveal que não vem, e a que aparece mas anima no
 * caminho.
 *
 * As seções sem reveal (`outros-cases`) entram na lista de propósito: se um dia
 * ganharem animação, é aqui que o gate acusa.
 */
/**
 * As seções que ainda usam o fade de seção inteira do `useSectionReveal`.
 *
 * A lista **encolhe a cada seção reescrita no molde novo**: nele o fade
 * de bloco não existe, cada elemento entra por conta própria (`useInView` com
 * `once: true`, AD-007). `empresas` saiu na v5. Quando a última sair, este
 * arquivo e o `useSectionReveal` saem junto.
 */
const SECOES_DA_V3: Record<string, string[]> = {
  // A home saiu inteira: todas as seções foram reescritas nos moldes novos
  // e animam por elemento (`useInView` com `once: true`, AD-007).
  // O `useSectionReveal` ainda serve `/projetos` e a case page.
  "/projetos": ["stack-transversal"],
  "/projetos/barbalog": ["outros-cases"],
};

interface MedidaDeSecao {
  id: string;
  opacity: string;
  property: string;
  duration: string;
...
/**
 * Roda no navegador. Declarada aqui porque as duas medições (com e sem
 * movimento reduzido) precisam ler exatamente as mesmas propriedades.
 */
function medirSecoes(ids: string[]): MedidaDeSecao[] {
  return ids.map((id) => {
    const node = document.querySelector(`[aria-labelledby="${id}"]`);
    if (!node) {
      return { id, opacity: "seção ausente", property: "", duration: "" };
    }
    const estilo = getComputedStyle(node);
    return {
      id,
      opacity: estilo.opacity,
      property: estilo.transitionProperty,
      duration: estilo.transitionDuration,
    };
  });
}

/**
 * "Não animar" tem duas formas legítimas e o teste precisa aceitar as duas:
 *
 * - `transition-property: none`, o que o `motion-reduce:transition-none` do
 *   Tailwind produz nas seções com reveal;
 * - `transition-duration: 0s`, o estado de quem nunca declarou transição
 *   nenhuma. O valor inicial de `transition-property` é **`all`**, não `none`:
 *   exigir `none` reprovaria a `outros-cases`, que não anima em momento algum.
 */
const anima = ({ property, duration }: MedidaDeSecao) =>
  property !== "none" && duration !== "0s";

test.describe("Reduced-motion nas seções da v3 (SEC-19)", () => {
  for (const [rota, ids] of Object.entries(SECOES_DA_V3)) {
    test(`as seções de ${rota} chegam prontas e sem transição`, async ({
      page,
    }) => {
      await page.emulateMedia({ reducedMotion: "reduce" });
      ...
      // `medirSecoes` é passada direto: o Playwright serializa a função e a
      // executa no navegador. Chamá-la de dentro de um arrow não funcionaria:
      // o identificador não existe do lado de lá.
      const medido = await page.evaluate(medirSecoes, ids);

      // A seção chega pronta: nada de esperar um reveal que não vem.
      expect(medido.map(({ id, opacity }) => ({ id, opacity }))).toEqual(
        ids.map((id) => ({ id, opacity: "1" })),
      );

      // E nada anima no caminho.
      expect(medido.filter(anima)).toEqual([]);
    });
  }

  /**
   * Controle positivo do teste acima: com movimento permitido, as seções que
   * ainda usam o mecanismo declaram transição. Sem isto, remover o reveal por
   * inteiro faria a auditoria passar por vacuidade.
   *
   * Aponta para `/projetos` e não para `/`: a home foi toda reescrita nos moldes
   * e nenhuma seção dela usa mais `useSectionReveal`.
   */
  test("com movimento permitido as seções que usam reveal animam", async ({
    page,
  }) => {
    await page.goto("/projetos");
    await page.waitForFunction(() => Boolean(window.__lenis));

    const ids = SECOES_DA_V3["/projetos"];
    const medido = await page.evaluate(medirSecoes, ids);

    expect(medido.map(({ id, property }) => ({ id, property }))).toEqual(
      ids.map((id) => ({ id, property: "opacity, transform" })),
    );
    expect(medido.every(anima), "nenhuma seção anima").toBe(true);
  });
});
